   ...
   public JSONObject confirmOrder(@RequestHeader("secret") String secret, @RequestBody JSONObject JsonBody, HttpServletResponse response) {
     ...
     if (goodId.indexOf("CS") != -1) {
       ...
       for (int i = 0; i < ArknightsApplication.CashGoodList.getJSONArray("goodList").size(); i++) {
         if (ArknightsApplication.CashGoodList.getJSONArray("goodList").getJSONObject(i).getString("goodId").equals(goodId)) {
       ...
       UserSyncData.getJSONObject("status").put("androidDiamond", Integer.valueOf(UserSyncData.getJSONObject("status").getIntValue("androidDiamond") + diamondNum));
       UserSyncData.getJSONObject("status").put("iosDiamond", Integer.valueOf(UserSyncData.getJSONObject("status").getIntValue("iosDiamond") + diamondNum));
     ...
     if (goodId.indexOf("GP_") != -1) {
       ...
       if (goodId.indexOf("GP_Once") != -1)
       {
         for (int j = 0; j < ArknightsApplication.GPGoodList.getJSONArray("oneTimeGP").size(); j++) {
           if (ArknightsApplication.GPGoodList.getJSONArray("oneTimeGP").getJSONObject(j).getString("goodId").equals(goodId)) {
