...
public class CommandMail
  ...
  public void processCommand(ICommandSender sender, String[] args) throws CommandException {
    if (args.length == 1) {
      ...
      if (args[1].equals("list")) {
        mailDao.queryMails().forEach(
            mail -> LOGGER.info("ID: " + mail.getId() + ", 邮件名: " + mail.getName() + ", 主题: " + mail.getSubject()));

        return;
      }

      LOGGER.error(
          "可用的子命令: create<创建邮件> setFrom<设置来源> setSubject<设置主题> setContent<设置内容> items<物品管理> info<查看信息> send<发送邮件>");
    } else {
  ...
  private void processInfoCommand(ICommandSender sender, String arg, List<Mail> mailList, String[] subCommandArgs) {
    ...
    JSONArray.parseArray(mail.getItems()).forEach(obj -> {
      ...
      if (item.getString("type").equals("CHAR")) {
        LOGGER.info(ArknightsApplication.characterJson.getJSONObject(item.getString("id")).getString("name") + " * "
            + item.getIntValue("count"));
      } else {
        LOGGER.info(ArknightsApplication.itemTable.getJSONObject(item.getString("id")).getString("name") + " * "
            + item.getIntValue("count"));
      }
    });
  }
  ...
  private void processItemsCommand(ICommandSender sender, String name, List<Mail> mailList, String[] args) {
    ...
    if (args.length == 0) {
      ...
      for (String id : ArknightsApplication.characterJson.keySet()) {
        if (ArknightsApplication.characterJson.getJSONObject(id).getString("name").equals(args[1])
            || id.equals(args[1])) {
          itemId = id;
          itemType = "CHAR";
        }
